...
<PreviewTitle>`lism-css`パッケージからimportする場合</PreviewTitle>
...
```zsh frame="none"
pnpm add https://pkg.pr.new/lism-ui/lism-ui/lism-css@{commitID}
```
</Note> */}
...
純粋なReactコンポーネントは、`lism-css`に含まれますが、Next.jsやAstroなどの特定のフレームワークに合わせたコンポーネントも準備しています。



### Astro

`lism-css/astro`パッケージを追加すると、`.astro`ファイルで構築されたコンポーネントを利用できるようになります。
...
<Preview>
	...
	<PreviewCode slot='tab' label='npm'>
	```bash frame="none"
	npm i lism-css/astro
	```
	</PreviewCode>
	<PreviewCode slot='tab' label='pnpm'>
	```bash frame="none"
	pnpm add lism-css/astro
	```
	</PreviewCode>
	<PreviewCode slot='tab' label='yarn'>
	```bash frame="none"
	yarn add lism-css/astro
	```
	</PreviewCode>
</Preview>


`lism-css/astro`から Lism Components をインポートして利用できるようになります。
...
`lism-css/astro`から`lism-css`のコードをインポートできるようにするため、`astro.config.js`の`vite.ssr.noExternal`に`"lism-css"`を追加してください。
</Callout>
...
<Callout type='warning'>
`@lism-ui/next`から`lism-css`のコードをインポートできるようにするため、`next.config.js`の`transpilePackages`に`"@lism-ui/next"`を追加してください。
</Callout>
