...
`<NavMenu.Link>`は、`data-lism-get`で`--pass-p`変数を受けとるようにしています。  
これにより、`<NavMenu.Root>`に(`--pass-p`)をセットして、paddingを一括調整できます。

<Reference>
[Lismで提案する pass-get について](/css/pass-get)
</Reference>

<Preview>
	<PreviewTitle>`--pass-p` でpaddingを一括指定</PreviewTitle>
	<PreviewArea p='30'>
		{/* <NavMenu.Root bd hasDivider style={{'--pass-p': 'var(--s30)'}}> */}
...
<Preview>
	...
	<PreviewCode>
		```jsx "Accordion.Root" "NavMenu.Nest" "data-lism-get='p'"
		<NavMenu.Root bd='y' hovBgc='base-2'>
			<NavMenu.Item>
				<NavMenu.Link hov='bgc:' href='#menu1'>Menu item</NavMenu.Link>
			</NavMenu.Item>
